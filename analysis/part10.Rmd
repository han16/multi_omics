...
selected_idx1 <- grep("PFC\\.[8-9]", names(dat_list))  
selected_idx2 <- grep("PFC\\.[10]", names(dat_list))
dat_pfc810 <- do.call(rbind, dat_list[c(selected_idx1, selected_idx2)]) # 107 x 13964  --> 107 x 12879  
...
screening_res <- screen(dat_pfc810, pv = scz3$pval.meta, p_thres = p_thres_screening, 
                                             num_genes = num_genes)
...
dat_pfc810 <- scale(dat_pfc810, scale = F)
...
res <- graphicalModel_range(dat_pfc810, 1:length(screening_res$primary), 
                                                  lambda_min = 0.01, lambda_max = 0.35, 
                                                  lambda_length = 30, verbose = T)
...
scale_vec_pfc810 <- sapply(res, function(x){compute_scale_free(as.matrix(x$adj_mat))})
# edges_vec_pfc35 <- sapply(res, function(x){sum(as.matrix(x$adj_mat))/2})
 idx <- which.max(scale_vec_pfc810) # choose tuning parameter lambda maximizing R^2 
# idx <- 27
 adj_pfc810 <- as.matrix(res[[idx]]$adj_mat)


 stopifnot(all(dim(adj_pfc810) == nrow(scz)))
...
# run the HMRF
set.seed(10)
seedindex <- rep(0, ncol(adj_pfc810))
...
hmrf_pfc810 <- hmrf(scz$pval.meta, adj_pfc810, seedindex, pthres = 0.1)  # pval must not be 0 or 1's, i.e between 0 and 1

#report_pfc35 <- covarianceSelection::report_results(scz$Gene, 1-hmrf_pfc35$post, scz$pval.meta, hmrf_pfc35$Iupdate)
report_pfc810 <- report_results(scz$Gene, 1-hmrf_pfc810$post, scz$pval.meta, hmrf_pfc810$Iupdate)
genes_pfc810 <- sort(as.character(report_pfc810$Gene[which(report_pfc810$FDR <= fdr_cutoff)]))
...
adj_pfc810 <- Matrix::Matrix(adj_pfc810, sparse = T)
#save(genes_pfc35, adj_pfc35, file="../data/gene_pfc35_20241203.RData")


report_pfc810_sort=report_pfc810[order(report_pfc810$FDR),]
report_pfc810_sort[1:length(genes_pfc810),]%>%
...
seed.gene=ifelse(report_pfc810_sort$Gene %in% scz3$Gene[scz3$pval.meta<0.01], "YES", "NO")
DAWN.gene=ifelse(report_pfc810_sort$Gene %in% genes_pfc810, "YES", "NO")
SCHEMA.FDR.gene=ifelse(report_pfc810_sort$Gene %in% scz1$Gene[scz1$qval.meta<0.05], "YES", "NO")
output_pfc810=data.frame(Gene=report_pfc810_sort$Gene, meta.pvalue=report_pfc810_sort$p.value, FDR=report_pfc810_sort$FDR, seed.gene=seed.gene, DAWN.gene=DAWN.gene, SCHEMA.FDR.gene=SCHEMA.FDR.gene)
#write.csv(output_pfc810, file="../data/output_pfc810_20241211.csv")
```