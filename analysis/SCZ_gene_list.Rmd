...
overlap2=length(intersect(genes_pfc35, DD_ID_gene))
overlap3=length(intersect(SCHEMA_FDR_gene, DD_ID_gene))
overlap4=length(intersect(dawn_new_gene, DD_ID_gene))

fisher_test=fisher.test(matrix(c(overlap2, overlap1, length(genes_pfc35), length(all_gene)), nrow=2))
OR[1]=fisher_test$estimate; pvalue[1]=fisher_test$p.value; pvalue_new_gene[1]=fisher.test(matrix(c(overlap4, overlap1, length(dawn_new_gene), length(all_gene)), nrow=2))$p.value

dawn_percentage[1]=overlap2/length(genes_pfc35)
all_gene_percentage[1]=overlap1/length(all_gene)
schema_fdr_percentage[1]=overlap3/length(SCHEMA_FDR_gene)
dawn_new_gene_percentage[1]=overlap4/length(dawn_new_gene)
