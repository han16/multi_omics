...
for (i in 2:4)
{
  ...
  OR[i-1]=fisher_test$estimate; pvalue[i-1]=fisher_test$p.value; 
  ...
  dawn_new_matrix=matrix(c(length(intersect(gene_set$DAWN_new_gene, gene_set[[i]])), 
                                        ...
                                        length(gene_set$all_gene)), nrow=2)
pvalue_new_gene[i-1]=fisher.test(dawn_new_matrix)$p.value
...
```{r, echo=F, message=F, warning=F}
OR_pvalue=data.frame(OR=OR, pvalue=round(pvalue,4), genes=c( "Autism genes", "DD/ID genes", "SCZ GWAS genes"))
...
###################### compute p value 
fisher.test(matrix(c(length(intersect(gene_set$DAWN_gene, gene_set$Autism_gene)), 
                                 length(intersect(gene_set$all_gene, gene_set$Autism_gene)), 
                                 length(gene_set$DAWN_gene), 
                                 length(gene_set$all_gene)), nrow=2))$p.value
fisher.test(matrix(c(length(intersect(gene_set$SCHEMA_FDR_gene, gene_set$Autism_gene)), 
                                 length(intersect(gene_set$all_gene, gene_set$Autism_gene)), 
