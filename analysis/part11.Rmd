...
selected_idx <- grep("PFC\\.[11-15]", names(dat_list))  
dat_pfc1115 <- do.call(rbind, dat_list[selected_idx]) # 107 x 13964  --> 107 x 12879  
...
screening_res <- screen(dat_pfc1115, pv = scz3$pval.meta, p_thres = p_thres_screening, 
                                             num_genes = num_genes)
...
dat_pfc1115 <- scale(dat_pfc1115, scale = F)
...
res <- graphicalModel_range(dat_pfc1115, 1:length(screening_res$primary), 
                                                  lambda_min = 0.01, lambda_max = 0.35, 
                                                  lambda_length = 30, verbose = T)
...
scale_vec_pfc1115 <- sapply(res, function(x){compute_scale_free(as.matrix(x$adj_mat))})
# edges_vec_pfc35 <- sapply(res, function(x){sum(as.matrix(x$adj_mat))/2})
 idx <- which.max(scale_vec_pfc1115) # choose tuning parameter lambda maximizing R^2 
# idx <- 27
 adj_pfc1115 <- as.matrix(res[[idx]]$adj_mat)


 stopifnot(all(dim(adj_pfc1115) == nrow(scz)))
...
# run the HMRF
set.seed(10)
seedindex <- rep(0, ncol(adj_pfc1115))
...
hmrf_pfc1115 <- hmrf(scz$pval.meta, adj_pfc1115, seedindex, pthres = 0.1)  # pval must not be 0 or 1's, i.e between 0 and 1

#report_pfc35 <- covarianceSelection::report_results(scz$Gene, 1-hmrf_pfc35$post, scz$pval.meta, hmrf_pfc35$Iupdate)
report_pfc1115 <- report_results(scz$Gene, 1-hmrf_pfc1115$post, scz$pval.meta, hmrf_pfc1115$Iupdate)
genes_pfc1115 <- sort(as.character(report_pfc1115$Gene[which(report_pfc1115$FDR <= fdr_cutoff)]))
...
adj_pfc1115 <- Matrix::Matrix(adj_pfc1115, sparse = T)
#save(genes_pfc35, adj_pfc35, file="../data/gene_pfc35_20241203.RData")


report_pfc1115_sort=report_pfc1115[order(report_pfc1115$FDR),]
report_pfc1115_sort[1:length(genes_pfc1115),]%>%
...
seed.gene=ifelse(report_pfc1115_sort$Gene %in% scz3$Gene[scz3$pval.meta<0.01], "YES", "NO")
DAWN.gene=ifelse(report_pfc1115_sort$Gene %in% genes_pfc1115, "YES", "NO")
SCHEMA.FDR.gene=ifelse(report_pfc1115_sort$Gene %in% scz1$Gene[scz1$qval.meta<0.05], "YES", "NO")
output_pfc1115=data.frame(Gene=report_pfc1115_sort$Gene, meta.pvalue=report_pfc1115_sort$p.value, FDR=report_pfc1115_sort$FDR, seed.gene=seed.gene, DAWN.gene=DAWN.gene, SCHEMA.FDR.gene=SCHEMA.FDR.gene)
#write.csv(output_pfc1115, file="../data/output_pfc1115_20241211.csv")
```